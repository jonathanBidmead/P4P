        ...
        if (is_initial_graph):
            if(msg_decoded[0:3] == "ADD"):
                ...
                if(agentExists == False):
                    file = open(init_layout_graph_path,"a")
            ...
            if(msg_decoded[0:3] == "DEL" or msg_decoded[0:3] == "OFF"):
                tempData = msg_decoded.split(",")
                file = open(init_layout_graph_path,"r")
                allText = file.readlines()
                for line in allText:#for each line in the file
                    elements = line.split(",")
...
f = open(init_layout_graph_path,"w")
f.write("")
f.close()
...
while True:
    ...
    if(datetime.datetime.now() - initTime > datetime.timedelta(seconds=10)):#TODO: make a better condition for switching from initialisation/setup to ongoing updates
