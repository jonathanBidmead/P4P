...
agent = smartServer.smartMqtt("EXIT_PLATFORM_1")#CHANGE
score = 10
...
def msg_func(client,userdata,msg):
    ...
    if(msg.topic == "/partBids"):
        if(msg_split[2] in machineCapabilites):
...
#add self to graph
agent.client.publish("/activeResources","ADD,EXIT_PLATFORM_1,BUFFER,13 0,KR10_3")#CHANGE
